...
class RegisterWorker:
    ...
    async def register_once(self, session: aiohttp.ClientSession) -> Tuple[str, str, str]:
        """执行一次注册，返回 (状态, 账号, 密码)"""
        # 1. 获取 SessionID（从 USESSIONID cookie）
        ...
        async with session.get(
            ...
            # 从 cookie 获取 USESSIONID 作为 captchaId
            sid = ""
            if 'USESSIONID' in session.cookie_jar.filter_cookies(reg_frame):
                sid = session.cookie_jar.filter_cookies(reg_frame)['USESSIONID'].value
